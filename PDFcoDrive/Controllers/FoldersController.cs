...
namespace PDFcoDrive.Controllers
{
	...
	public class FoldersController : Controller
	{
		...
		public async Task<IActionResult> Browse(int id, string? returnUrl)
		{
			...
			if (!await _folders.CanAccessAsync(folder, CurrentUserId, CurrentRole))
				return Forbid();

			var allChildren = await _folders.GetChildrenAsync(id);
			var children = new List<Folder>();
			foreach (var child in allChildren)
			{
				if (await _folders.CanAccessAsync(child, CurrentUserId, CurrentRole))
					children.Add(child);
			}
		...
		public async Task<IActionResult> Create(int? parentId, string name, string? description, AccessLevel access, string icon, string color, List<string>? allowedUserIds, List<int>? allowedGroupIds, string? returnUrl)
		{
			...
			var folder = new Folder
			{
				...
				AllowedUserIds = access == AccessLevel.Team && allowedUserIds != null
					? allowedUserIds
					: new List<string>(),
				AllowedGroupIds = access == AccessLevel.Group && allowedGroupIds != null
					? allowedGroupIds
					: new List<int>()
			};
		...
		[HttpPost]
		[ValidateAntiForgeryToken]
		public async Task<IActionResult> Edit(int id, string name, string? description, AccessLevel access, string icon, string color, List<string>? allowedUserIds, List<int>? allowedGroupIds, string? returnUrl)
		{
			var folder = await _folders.GetByIdAsync(id);
			if (folder == null) return NotFound();
			if (!_folders.CanManageFolder(folder, CurrentUserId, CurrentRole))
			...
			folder.AllowedUserIds = access == AccessLevel.Team && allowedUserIds != null
				? allowedUserIds
				: new List<string>();
			folder.AllowedGroupIds = access == AccessLevel.Group && allowedGroupIds != null
				? allowedGroupIds
				: new List<int>();
		...
		// ============================================
		// مشاهده پوشه‌های کاربر عادی (فقط Admin)
		// ============================================
		[HttpGet]
